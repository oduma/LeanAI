...
public class SaveFoodLogCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_CallsDeleteThenAddWithCorrectEntities()
    {
        var deleteOrder  = 0;
        var addOrder     = 0;
        var callSequence = 0;
        ...
        await _handler.Handle(new SaveFoodLogCommand(TestDate, TwoItems), CancellationToken.None);

        deleteOrder.Should().BeLessThan(addOrder, "DeleteByDate must be called before AddRange");
    ...
    [Fact]
    public async Task Handle_WithEmptyItemsList_DeletesAndAddsEmpty()
    {
        await _handler.Handle(new SaveFoodLogCommand(TestDate, []), CancellationToken.None);
